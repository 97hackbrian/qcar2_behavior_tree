...
class QCar2BehaviorTreeManager(Node):
    def __init__(self) -> None:
        ...
        self.current_goal: GoalPoint | None = None
        self.current_goal_start_sec: float | None = None
        ...
        self.last_motor_cmd: MotorCommands | None = None
        ...
        self.get_logger().info(
            f'BehaviorTree manager iniciado con {len(self.goals)} goals | tick_hz={tick_hz:.1f}'
        )
    ...
    def _load_goals_from_parameters(self) -> list[GoalPoint]:
        goals_raw: list[list[float]] = [
    ...
    def _on_stop_sign(self, msg: StopSignDetection) -> None:
        self.stop_sign_detected = bool(msg.detected)

    def _on_traffic_light(self, msg: TrafficLightDetection) -> None:
        self.traffic_light_detected = bool(msg.detected)
        self.traffic_light_state = (msg.state or 'unknown').lower()

    def _on_zebra(self, msg: ZebraCrossingDetection) -> None:
        self.zebra_detected = bool(msg.detected)

    def _on_mixer_state(self, msg: String) -> None:
        self.mixer_state = msg.data

    def _on_motor_cmd(self, msg: MotorCommands) -> None:
    ...
    def _update_tf_flags_from_message(self, msg: TFMessage) -> None:
    ...
    def _update_tf_chain(self) -> None:
        try:
    ...
    def _build_tree_from_parameters(self) -> BTNode:
    ...
    def _parse_step(self, token: str, index: int) -> BTNode:
        if token.startswith('wait:'):
            _, value = token.split(':', 1)
            return Wait(f'wait_{index}', float(value))
        ...
        return Action(f'noop_{index}', lambda c: Status.SUCCESS)
    ...
    def _is_tf_chain_ready(self, _: TickContext | None) -> bool:
    ...
    def _action_publish_runtime_state(self, _: TickContext) -> Status:
        state = (
            f'RUN idx={self.current_goal_index} '
            f'person={self.person_detected} stop={self.stop_sign_detected} '
            f'zebra={self.zebra_detected} tl={self.traffic_light_state} mixer={self.mixer_state}'
        )
    ...
    def _action_dispatch_next_goal(self, _: TickContext) -> Status:
        ...
        self._publish_state(f'GOAL_DISPATCHED index={self.current_goal_index}')
        return Status.SUCCESS

    def _action_wait_goal_reached_or_timeout(self, _: TickContext) -> Status:
        ...
        if self._goal_reached():
            self._publish_state(f'GOAL_REACHED index={self.current_goal_index}')
            self.current_goal = None
            self.current_goal_start_sec = None
            return Status.SUCCESS

        if self._goal_timeout():
            self._publish_state(f'GOAL_TIMEOUT index={self.current_goal_index} -> continue')
            self.current_goal = None
            self.current_goal_start_sec = None
            return Status.SUCCESS
    ...
    def _action_check_all_goals_done(self, _: TickContext) -> Status:
    ...
    def _is_goal_terminal_state(self) -> bool:
        return False
